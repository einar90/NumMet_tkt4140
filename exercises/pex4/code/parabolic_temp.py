...
def explicit_python_solver(u_left=1.0, u_right=0.0,
                           nx=20, r=0.5,
                           xmin=0.0, xmax=1.0,
                           tmin=0.0, tmax=1.0, k=1.0):
    ...
    u[0] = u_left
    u_old[-1] = u_right
    dt = r*dx**2/k  # compute timestep based on Fourier number, dx, diffusivity
...
theta = 0.75  # implicitness: theta=0.5 Crank-Nicholson , theta=1.0 implicit
call = functools.partial
solverlist = [explicit_python_solver]

solvers = [
    [call(solverlist[0],
          u_left=100.0, u_right=0.0,
          nx=nx, r=0.5,
          xmin=0.0, xmax=L,
          tmin=0.0, tmax=tmax,
          k=1.0)],
